...
export const createBooking = async (req, res) => {
    try {
        ...
        const { userId, selectedDate, selectedTime,note } = req.body;
        ...
        if (existingBooking) {
            return res.status(400).json({ success: false, message: 'Bạn có lịch hẹn vào ngày này' });
        }
        ...
        res.status(201).json({ success: true, message: 'Đặc lịch thành công',data: booking });
    } catch (error) {
...
export const getAll = async (req, res) => {
    try {
        const bookings = await Booking.find({isApproved:'pending'});
        res.status(200).json({ success: true, data: bookings });
    } catch (error) {
        console.error('Error fetching bookings:', error);
        res.status(500).json({ success: false, message: 'An error occurred while fetching bookings' });
...
export const deleteBooking = async (req,res)=>{
    ...
    try {
        ...
        res.status(200).json({success:true,message:"Xóa lịch hẹn thành công",data: deleteBooking});
    } catch (error) {
