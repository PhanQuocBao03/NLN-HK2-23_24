...
const UserDelete = ({ open, handleClose, userId }) => {
  const { data, loading, error } = useFetchData(`${BASE_URL}/users/${userId}`);
  console.log('checkdl',data)
  ...
  return (
    ...
    >
      ...
      <DialogBody className="p-4">
        ...
        {data && (
            <div>
                <p>Bạn có muốn xóa khách hàng <span className="text-[18px] font-semibold text-primaryColor">{data.name}</span>?</p>
            </div>
        )}
